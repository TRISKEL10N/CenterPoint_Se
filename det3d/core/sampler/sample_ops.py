        ...
        if len(sampled) > 0:
            ...
            for info in sampled:
                try:
                    ...
                    ).reshape(-1, num_point_features - 1)
                    #gt里的点属性仍为5，需将num_point_features<6> - 1
                    #the number of point feature is 5, change "num_point_features" to "num_point_features - 1"
                    ...
                    #为s_points的语义位赋零
                    #padding the semantic place with 0
                    s_points = np.concatenate((s_points, np.zeros((s_points.shape[0], 1))), axis=1)
                    s_points_list.append(s_points)
                    # print(pathlib.Path(info["path"]).stem)
                except Exception:
                    #print(str(pathlib.Path(root_path) / info["path"]))
                    with open('/mnt/cephfs/home/yangjiahao/CenterPoint/运行报错.txt','a') as f:
