...
def add_semantic(info, points):
    # 给点云添加语义信息
    #add semantic information to the point cloud
    #lidarpath2setoken = get_token()
    import ast
    with open('/mnt/cephfs/home/yangjiahao/CenterPoint/lidarpath2setoken.txt', 'r') as f:
        lidarpath2setoken = f.read()
    #print('Read Done!')
    lidarpath2setoken = ast.literal_eval(lidarpath2setoken)

    if info['lidar_path'] in list(lidarpath2setoken.keys()):
        setoken = lidarpath2setoken.get(str(info['lidar_path']))
    
    semantic_path = os.path.join('/mnt/cephfs/home/yangjiahao/CenterPoint/semantic_pred/' + str(setoken) + '_lidarseg.bin')
    semantics = np.fromfile(semantic_path, dtype=np.float32).reshape(-1, 1)
    
    #按行连接两个numpy数组
    #connect the two numpy array by row
    points = np.concatenate((points, semantics), axis=1)
...
class LoadPointCloudFromFile(object):
    ...
    def __call__(self, res, info):
        ...
        if self.type == "NuScenesDataset":
            ...
            #添加语义信息
            #add the semantic information
            points = add_semantic(info, points)
            #print('type of points: ' + str(type(points)))
            #print('shape of points: ' + str(points.shape))

            sweep_points_list = [points]
            #print('type of sweep_points_list: ' + str(type(sweep_points_list)))
            #print('sweep_points_list: ' + str(sweep_points_list))
            ...
            for i in np.random.choice(len(info["sweeps"]), nsweeps - 1, replace=False):
                ...
                #print('type of points_sweep: ' + str(type(points_sweep)))
                #print('shape of points_sweep: ' + str(points_sweep.shape))
                #为sweep中的点云的语义位赋零
                #padding the semantic place of the point cloud in sweeps with 0
                points_sweep = np.concatenate((points_sweep, np.zeros((points_sweep.shape[0], 1))), axis=1)
